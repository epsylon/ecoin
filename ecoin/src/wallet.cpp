...
// ecoin: optional setting to unlock wallet for block minting only;
                                  ...
                                  vector<char>& vfBest, int64& nBest, int iterations = 1000)
{
    ...
    for (int nRep = 0; nRep < iterations && nBest != nTargetValue; nRep++)
    {
        ...
        for (int nPass = 0; nPass < 2 && !fReachedTarget; nPass++)
        {
            ...
            {
                if (nPass == 0 ? GetRand(2) : !vfIncluded[i])
                {
...
// ecoin: total coins staked (non-spendable until maturity)
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                // ecoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-ecoin-address
...
// Ecoin: get current stake weight
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CEcoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CEcoinAddress(address).ToString());
}
...
// ecoin: check 'spent' consistency between wallet and txindex
// ecoin: fix wallet spent state according to txindex
...
// ecoin: disable transaction (only for coinstake)
